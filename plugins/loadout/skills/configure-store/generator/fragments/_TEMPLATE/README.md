...
5. flavor별 차이가 필요할 때만: 조각 문구는 `fragment.codex.md`(codex에서 그걸 사용, 없으면 공용), 딸린 파일은 `files.codex/`(codex flavor에서만 추가 복사). 카탈로그 표시명이 codex에서 달라야 하면 meta.json에 `"label_codex": "<codex용 표시명>"`(없으면 `label` 공용). 특정 하네스 전용 품목이면 meta.json에 `"flavors": ["claude"]` — 다른 flavor에선 설치 거부된다.
